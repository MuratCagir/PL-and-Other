CREATE OR REPLACE package body ETL_UTL.pl
as
------------------------------------------------------------------------------
--  Created By              : Erhan BEKTAŞ
--  Creation Date           : 20201214
--  Last Revision Date      : 
--  Package Name            : ETL_UTL.PL
--  Package Version         : 1.0
--  Package Description     : 
...
------------------------------------------------------------------------------
  ...
  -- exceptions
  subpartition_not_found   exception;
  partition_not_found      exception;
  table_not_partitioned    exception;

  pragma exception_init(subpartition_not_found,-20169);
  ...
  -- eg. pl.truncate_table('ETL_UTL.LOGS')
  ----------------------------------------------------------------------------------------------
  ...
  -- eg. pl.truncate_table('ETL_UTL.LOGS') ignores errors if table not found
  ----------------------------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------------------------
  procedure truncate_subpartition(i_owner varchar2, i_table varchar2, i_partition varchar2, i_subpartition varchar2)
  is
    v_proc varchar2(1000) := gv_package||'.truncate_subpartition';
    v_cnt  number := 0;
  begin
    gv_sql := '
      select count(1)
      ...
      where
        table_name = upper('''||i_table||''') and
        table_owner = upper('''||i_owner||''')      and
        partition_name = upper('''||i_partition||''')      and
        subpartition_name = upper('''||i_subpartition||''')
    ';
    execute immediate gv_sql into v_cnt;

    if v_cnt = 0 then
      raise subpartition_not_found;
    else
      gv_sql := 'alter table '|| i_owner||'.'||i_table||' truncate subpartition '||i_subpartition;
      execute immediate gv_sql;
      logger.success( 'Subpartition '||i_subpartition||' truncated', gv_sql);
    end if;

  exception
    when subpartition_not_found then
      logger.error(v_proc||'Subpartition '||i_subpartition||' not found!', gv_sql);
      raise_application_error (
        -20169,
        v_proc||'Subpartition '||i_subpartition||' not found!'
      );
    when others then
      logger.error(sqlerrm, gv_sql);
      raise;
  ...
  ----------------------------------------------------------------------------------------------
  -- truncates given partitions starting from date through number of partitions,
  ...
  begin
    ...
    loop
      ...
      exit when (v_range_type in ('D', 'd') and v_max_date-1 > i_date)
           or (v_range_type = 'm' and add_months(v_max_date,-1) > i_date)
           or (v_range_type = 'y' and add_months(v_max_date,-12) > i_date);
  ...
  is
    ...
    v_date date;
    v_sp_count number;
    v_sp_min_suffix varchar2(100);
    v_sp_suffix varchar2(100);
    v_sphv varchar2(100);
    v_max_nvpn varchar2(100);
  begin
    ...
    -- Control step to find whether the table has subpartitions or not
    select
      /*+ parallel(32) */
      count(1)
    into
    ...
    if v_sp_count = 0 then
      ...
      -- Find the last partition name
      select
        distinct
        nth_value(partition_name, 1) over (order by partition_name desc rows between unbounded preceding and unbounded following) as nvpn
        into v_max_nvpn
      from (
        select
          partition_name,
      ...
      -- Add the next partition with the subpartition that has the min high value
      gv_sql :=  'alter table '||i_owner||'.'||i_table||' add partition '||v_part_name||
      ' values less than (
          '||find_next_high_value(v_range_type, v_high_value)||'
          )(
      ...
      -- Add the rest of the subpartitions to the partition
      for i in(
        select
          ltrim(subpartition_name, partition_name) as subpart_suffix,
          extractvalue
            ( dbms_xmlgen.getxmltype
            ( 'select high_value
            from dba_tab_subpartitions where table_name = '''||dts.table_name||''' and partition_name = '''||dts.partition_name||''' and  subpartition_name = '''||dts.subpartition_name||''''),
            '//text()' ) as high_value
        from
          dba_tab_subpartitions dts
        where
          table_owner = i_owner
          and table_name = i_table
          and partition_name = v_max_nvpn
        order by
          subpart_suffix asc
      )
      loop
        gv_sql := gv_sql || ' subpartition '||v_part_name||'_'||i.subpart_suffix||' values ('||i.high_value||'),
        ';
        printl(gv_sql);
      end loop;
      ...
      begin
        ...
        exception
        when others then
          gv_sql := replace(gv_sql, 'values (', 'values less than (');
          execute immediate gv_sql;
      end;
  ...
  is
    v_range_type varchar2(2) := find_partition_range_type(i_owner, i_table);
  begin
  ...
  begin
    ...
    if i_part_name is null then
      ...
      execute immediate v_sql using v_owner, v_table;
    else
      v_sql := '
        begin
          dbms_stats.gather_table_stats(
      ...
      execute immediate v_sql using v_owner, v_table, i_part_name;
    end if;
  end;
